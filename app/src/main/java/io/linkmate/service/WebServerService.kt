    ...
    private fun observePortAndRunServer() {
        ...
        portCollectJob = serviceScope.launch {
            webServerConfigRepository.portFlow().collectLatest { port ->
                ...
                updateNotification("Web 服务运行�? 端口 $port")
                restartServer(port)
            }
        }
    }
            ...
            routing {
                ...
                get("/") {
                    ...
                    try {
                        ...
                        var authed = false
                        try {
                            // 直接�?HTTP headers 读取 Cookie，避免使�?Ktor cookies API 可能导致的阻�?
                                ...
                                <html>
                                  ...
                                  <body>
                                    ...
                                    <form method="post" action="/add_reminder_form">
                                      <div>
                                        <label>提醒内容:</label>
                                        <input type="text" name="message" placeholder="请输入提醒内�? required style="width: 300px;" />
                                      </div>
                ...
                post("/login") {
                    ...
                    try {
                        ...
                        call.respond(HttpStatusCode.InternalServerError, "服务器错�? ${e.message}")
                    }
                }
                ...
                post("/config") {
                    ...
                    try {
                        ...
                        // 更新应用设置（和�?API Key、靠近唤醒、GPS 和天气刷新间隔）
                ...
                post("/add_reminder_form") {
                    ...
                    try {
                        ...
                        if (message.isBlank()) {
                            call.respondText("<html><body><h1>错误：提醒内容不能为�?/h1><a href=\"/\">返回</a></body></html>", contentType = ContentType.Text.Html)
                            return@post
                        }
